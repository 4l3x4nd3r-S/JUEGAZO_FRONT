      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {canchas.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground text-lg">No se encontraron canchas con los filtros aplicados</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {canchas.map((cancha) => (
              <div key={cancha.id} className="bg-card rounded-xl shadow-lg overflow-hidden border border-border">
                {/* Image */}
                <div className="h-48 bg-muted relative">
                  ...
                  {cancha.techada && (
                    <div className="absolute top-3 right-3">
                      <Shield className="h-6 w-6 text-primary bg-card rounded-full p-1 shadow-md" />
                    </div>
                  )}
                </div>
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-xl font-semibold text-foreground">{cancha.nombre}</h3>
                    <span className="text-2xl font-bold text-primary">S/.{cancha.precio}</span>
                  </div>
                  ...
                  {cancha.descripcion && (
                    <p className="text-muted-foreground text-sm mb-4 line-clamp-2">{cancha.descripcion}</p>
                  )}
